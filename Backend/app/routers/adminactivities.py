...
from app.repository import admin, company, roles, users, riders, foods, orders, feedbacks, user_role, payments, costs
...
async def update(id: int, request: schemas.ShowAdmin, db: Session = Depends(get_db), current_user: schemas.User = Security(
    oauth2.get_current_active_user,
    scopes=[Role.SUPER_ADMIN["name"], Role.ADMIN["name"], Role.COOK["name"],
            Role.ORDER_MANAGER["name"],  Role.ACCOUNT_MANAGER["name"]],
)):
...
async def create_food(name: str = Form(...),
    ...
    food = schemas.Food(name=name, ingredients=ingredients,
                        price=float(price), addedBy=current_user.id)
...
@router.put('/food/update/{id}', status_code=status.HTTP_202_ACCEPTED, response_model=schemas.ShowFood,  tags=['Admins', 'Super Admin'])
...
@router.put('/food/update/nofile/{id}', status_code=status.HTTP_202_ACCEPTED, response_model=schemas.ShowFood,  tags=['Admins', 'Super Admin'])
async def updateWithoutFile(
        id: int,
        name: str = Form(...),
        ingredients: str = Form(...),
        price: str = Form(...),
    ...
    food = schemas.Food(name=name, ingredients=ingredients,
                        price=float(price), addedBy=current_user.id)
    return foods.updateWithNoFile(id, food, db)
...
async def create_bulk_user(
    ...
    readInputFile = csv.DictReader(codecs.iterdecode(
        csvFile.file, 'ISO-8859-1'), skipinitialspace=True, delimiter=",")

    return createBulk(readInputFile, name, db)


# Payments
@router.post('/payment/pay',  tags=['Admins', 'Accountant', 'Super Admin'])
async def create_payment(request: schemas.CreatePayment, db: Session = Depends(get_db), current_user: schemas.User = Security(
    oauth2.get_current_active_user,
    scopes=[Role.SUPER_ADMIN["name"], Role.ADMIN["name"],
            Role.ACCOUNT_MANAGER["name"]],
)):
    request.modifyBy = current_user.id
    return payments.create(request, db)


@router.get('/payment/{id}', response_model=schemas.ShowPayment,  tags=['Admins', 'Accountant', 'Super Admin'])
async def get_payment(id: int,
    ...
    return payments.show(id, db)


@router.get('/payment/',  response_model=List[schemas.ShowPayment],  tags=['Admins', 'Accountant', 'Super Admin'])
async def all(db: Session = Depends(get_db), current_user: schemas.User = Security(
    oauth2.get_current_active_user,
    scopes=[Role.SUPER_ADMIN["name"], Role.ADMIN["name"],
            Role.ACCOUNT_MANAGER["name"]],
)):
    return payments.get_all(db)


@router.put('/payment/update/{id}', status_code=status.HTTP_202_ACCEPTED, response_model=schemas.ShowPayment,  tags=['Admins', 'Accountant', 'Super Admin'])
async def update(id: int, request: schemas.ShowPayment, db: Session = Depends(get_db), current_user: schemas.User = Security(
    oauth2.get_current_active_user,
    scopes=[Role.SUPER_ADMIN["name"], Role.ADMIN["name"],
            Role.ACCOUNT_MANAGER["name"]],
)):
    request.modifyBy = current_user.id
    return payments.update(id, request, db)


# Cost
@router.post('/cost/pay',  tags=['Admins', 'Accountant', 'Super Admin'])
async def create_cost(request: schemas.CreateCost, db: Session = Depends(get_db), current_user: schemas.User = Security(
    oauth2.get_current_active_user,
    scopes=[Role.SUPER_ADMIN["name"], Role.ADMIN["name"],
            Role.ACCOUNT_MANAGER["name"]],
)):
    return costs.create(request, db)


@router.get('/cost/{id}', response_model=schemas.ShowCost,  tags=['Admins', 'Accountant', 'Super Admin'])
async def get_cost(id: int, db: Session = Depends(get_db), current_user: schemas.User = Security(
    oauth2.get_current_active_user,
    scopes=[Role.SUPER_ADMIN["name"], Role.ADMIN["name"],
            Role.ACCOUNT_MANAGER["name"]],
)):
    return costs.show(id, db)


@router.get('/cost/',  response_model=List[schemas.ShowCost],  tags=['Admins', 'Accountant', 'Super Admin'])
async def all(db: Session = Depends(get_db), current_user: schemas.User = Security(
    oauth2.get_current_active_user,
    scopes=[Role.SUPER_ADMIN["name"], Role.ADMIN["name"],
            Role.ACCOUNT_MANAGER["name"]],
)):
    return costs.get_all(db)


@router.put('/cost/update/{id}', status_code=status.HTTP_202_ACCEPTED, response_model=schemas.ShowCost,  tags=['Admins', 'Accountant', 'Super Admin'])
async def update(id: int, request: schemas.ShowCost, db: Session = Depends(get_db), current_user: schemas.User = Security(
    oauth2.get_current_active_user,
    scopes=[Role.SUPER_ADMIN["name"], Role.ADMIN["name"],
            Role.ACCOUNT_MANAGER["name"]],
)):
    return costs.update(id, request, db)
